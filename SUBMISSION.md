...
more verifiable than a number I assert — you can check the arithmetic without
trusting my client.
...
and no one — including me — is in a position to see them early.

The nuance I care about is that **not everything should be hidden.** A market
...
it, my own sync gate waited on *all three* sub-wallets — shielded, unshielded,
and dust — while only the unshielded scan was relevant, so the deploy deadlocked
against a balance it had already found.

**A privacy bug that only showed up under review.** See "What I learned".

**Being unable to test the thing I built.** Live mode needs a proof server, and
...
**The most valuable thing I did was review the contract against Midnight's own
checklists, and it found a real bug.**

I ran `prediction-market.compact` against the Midnight Expert review skills
...
total, because I had protected the value and not the *linkage*. Privacy is not a
...
Three smaller things I now believe:

- **A blinded tag means my own client cannot recognise its own positions from
...
- **`persistentHash` is not a commitment.** It provides binding, not hiding, and
  does not clear witness taint. `persistentCommit` is the primitive; I had
