 ...
 *
 * Taken from midnight-leaderboard, which took it from the bboard example.
 * midnight-js requires a PrivateStateProvider and the Node one is LevelDB-backed,
 * which does not exist in a browser.
 *
 * In-memory means the witness state does not survive a reload -- which is fine,
 * because the part that MUST survive is the stake and salt, and those are kept
 * separately in `notes-store.ts` where they can be persisted and exported.
 * Losing a salt forfeits a position permanently; losing this cache does not.
 */
