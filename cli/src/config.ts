...
const remoteEnvironment = (
  network: NetworkName,
  host: string,
): new (logger: Logger) => RemoteTestEnvironment =>
  class extends RemoteTestEnvironment {
    getEnvironmentConfiguration(): EnvironmentConfiguration {
      ...
      return {
        ...
        networkId: network,
        indexer: `https://indexer.${host}.midnight.network/api/v4/graphql`,
        indexerWS: `wss://indexer.${host}.midnight.network/api/v4/graphql/ws`,
        node: `https://rpc.${host}.midnight.network`,
        nodeWS: `wss://rpc.${host}.midnight.network`,
        faucet: `https://midnight-tmnight-${host}.nethermind.dev/`,
        proofServer: container.getUrl(),
      };
    }
  };
...
const makeConfig = (network: NetworkName): Config => ({
  ...
  getEnvironment(logger: Logger): TestEnvironment {
    ...
    const Env = remoteEnvironment(network, network);
